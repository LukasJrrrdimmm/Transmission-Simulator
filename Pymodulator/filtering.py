...
class Demodulations:
	...
	def De_MASK_Entrelac_TV(signal, d):
		x = signal.real
		y = signal.imag
		print(pd.DataFrame({"X":x, "Y":y}))
		xdm = []
	...
	def De_MASK_Entrelac_TH(signal, d):
		x = signal.real
		y = signal.imag
		print(pd.DataFrame({"X":x, "Y":y}))
		xdm = []
...
#Próximos Passos: Modulação MQPM, Atenuação ou redução de ruído, simulação de ruidos térmico e de túnel
